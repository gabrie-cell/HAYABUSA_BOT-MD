...
const handler = async (m, { conn, text, command }) => {
  if (!text) return m.reply(`💫 *Hayabusa invocando*
  ...
  try {
    ...
    if (!text.startsWith("https://")) {
      ...
      if (!res?.videos?.length) {
        return m.reply(`🔍 *Hayabusa buscando*
...
❌ Nada fue encontrado…`)
    ...
    if (isAudio) {
      ...
      await m.reply(`🔍 *hayabusa — Análisis*
...
• *.play2 ${url}*`)
    ...
    await m.reply(`❌ *Hayabusa — Error en la operación*
...
const downloadMedia = async (conn, m, url, title, thumbnail, type) => {
  try {
    ...
    const msg = `🎶 *Hayabusa — Descarga en curso*
...
🌌 Preparando tu ${type === "mp3" ? "audio espera un poco..." : "video"}...`
    ...
    await conn.sendMessage(
      m.chat,
      {
        text: `🎶 *Hayabusa — Operación completada*

✨ *Título:* ${fileTitle}
🌌 Entregada completa xd.`,
        edit: sent.key
      }
    )
    ...
    await m.reply(`❌ *Hayabus — Falla en la entrega*
